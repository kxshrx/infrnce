...
class LogClassifier:
    ...
    async def _load_bert_model(self):
        ...
        try:
            ...
            model_path = Path("../log_classification_system/models/controlled_bert_model.pth")
            if not model_path.exists():
                model_path = Path("/Users/kxshrx/dev/log-classification/log_classification_system/models/controlled_bert_model.pth")
            
            if not model_path.exists():
                logger.warning(f"BERT model not found at {model_path}. BERT classification will be skipped.")
                self.bert_loaded = False
                return
...
"""
    ...
    async def _classify_with_bert(self, log_text: str) -> Tuple[Optional[str], float]:
        ...
        try:
            ...
            predicted_category = self.bert_label_mapping.get(predicted_label.item(), "Unknown")
