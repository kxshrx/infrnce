...
async def health_check():
    ...
    global classifier
    return {
        "status": "healthy" if classifier and classifier.is_initialized else "unhealthy",
...
async def classify_log(request: LogClassificationRequest):
    ...
    try:
        ...
        logger.info(f"Classified log in {processing_time_ms}ms via {result['stage']}: {result['category']}")
        return response
...
async def generate_synthetic_log():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Log generation failed: {str(e)}"
        )
