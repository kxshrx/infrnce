...
def create_production_demo_from_complete_results(df_complete, target_size=2000):
    ...
    unclassified_target = target_size - regex_target - bert_target - llm_target  # Remaining = 220 logs
    ...
    print(f"  Unclassified: {unclassified_target} logs ({unclassified_target/target_size*100:.1f}%)")
    print(f"=" * 60)
    ...
    if len(regex_classified) > 0:
        ...
        instance_mgmt = regex_classified[regex_classified['final_category'] == 'Instance_Management_Compute']
        if len(instance_mgmt) > 0:
            # Limit to ~35% of total dataset (700 logs) with most coming from regex
            regex_sample.append(instance_mgmt.sample(n=min(400, len(instance_mgmt)), random_state=42))
        
        # System_Operations_LibVirt (second most common - increase representation)
        system_ops = regex_classified[regex_classified['final_category'] == 'System_Operations_LibVirt']
        if len(system_ops) > 0:
            regex_sample.append(system_ops.sample(n=min(300, len(system_ops)), random_state=42))
        
        # Boost specific underrepresented categories in regex
        boost_categories_regex = ['Configuration_Errors', 'File_System_Errors', 'Resource_Management']
        for boost_cat in boost_categories_regex:
            boost_logs = regex_classified[regex_classified['final_category'] == boost_cat]
            if len(boost_logs) > 0:
                ...
                if current_total < regex_target:
                    ...
                    regex_sample.append(boost_logs.sample(n=boost_size, random_state=42))
        
        # Fill remaining with other categories proportionally
        remaining_categories = regex_classified[
            ~regex_classified['final_category'].isin(['Instance_Management_Compute', 'System_Operations_LibVirt', 
                                                    'Configuration_Errors', 'File_System_Errors', 'Resource_Management'])
        ]
        if len(remaining_categories) > 0:
            remaining_needed = regex_target - sum(len(s) for s in regex_sample)
            if remaining_needed > 0:
                regex_sample.append(remaining_categories.sample(n=min(remaining_needed, len(remaining_categories)), random_state=42))
        
        regex_final = pd.concat(regex_sample).head(regex_target)
        demo_sample.append(regex_final)
        print(f"✅ Regex showcase: {len(regex_final)} logs from {len(regex_final['final_category'].unique())} categories")
    ...
    if len(bert_classified) > 0:
        ...
        if len(high_conf) > 0:
            ...
            if len(network_ops) > 0:
                bert_sample.append(network_ops.sample(n=min(120, len(network_ops)), random_state=42))
            
            # Scheduler_Operations (increase representation)
            scheduler_ops = high_conf[high_conf['final_category'] == 'Scheduler_Operations']
            if len(scheduler_ops) > 0:
                bert_sample.append(scheduler_ops.sample(n=min(130, len(scheduler_ops)), random_state=42))
            
            # Other high confidence categories (including some Instance_Management for diversity)
            other_high_conf = high_conf[
                ~high_conf['final_category'].isin(['Network_Operations', 'Scheduler_Operations'])
            ]
            if len(other_high_conf) > 0:
                ...
                if remaining_high > 0:
                    ...
                    instance_mgmt_bert = other_high_conf[other_high_conf['final_category'] == 'Instance_Management_Compute']
                    if len(instance_mgmt_bert) > 0 and remaining_high >= 50:
                        bert_sample.append(instance_mgmt_bert.sample(n=min(50, len(instance_mgmt_bert)), random_state=42))
                        remaining_high -= 50
                    
                    # Boost underrepresented categories in BERT
                    boost_categories_bert = ['Configuration_Errors', 'File_System_Errors', 'Network_Connection_Errors', 'Resource_Management']
                    for boost_cat in boost_categories_bert:
                        boost_logs = other_high_conf[other_high_conf['final_category'] == boost_cat]
                        if len(boost_logs) > 0 and remaining_high >= 20:
                            boost_size = min(20, len(boost_logs), remaining_high)
                            bert_sample.append(boost_logs.sample(n=boost_size, random_state=42))
                            remaining_high -= boost_size
                    
                    # Fill rest with other categories
                    other_cats = other_high_conf[~other_high_conf['final_category'].isin([
                        'Instance_Management_Compute', 'Configuration_Errors', 'File_System_Errors', 
                        'Network_Connection_Errors', 'Resource_Management'
                    ])]
                    if len(other_cats) > 0 and remaining_high > 0:
                        bert_sample.append(other_cats.sample(n=min(remaining_high, len(other_cats)), random_state=42))
        ...
        if len(med_conf) > 0:
            ...
            bert_sample.append(med_conf.sample(n=min(target_med, len(med_conf)), random_state=42))
        ...
        if len(low_conf) > 0:
            ...
            if target_low > 0:
                bert_sample.append(low_conf.sample(n=min(target_low, len(low_conf)), random_state=42))
        
        bert_final = pd.concat(bert_sample).head(bert_target)
        demo_sample.append(bert_final)
        print(f"✅ BERT showcase: {len(bert_final)} logs with {len(bert_final['final_category'].unique())} categories")
    ...
    if len(llm_classified) > 0:
        ...
        error_subcats = llm_classified[llm_classified['final_category'].str.contains('Error|Timeout', na=False)]
        if len(error_subcats) > 0:
            target_errors = int(llm_target * 0.5)  # Reduced to 210 logs to make room for other categories
            error_categories = error_subcats['final_category'].value_counts()
            
            # Prioritize underrepresented error categories
            priority_errors = ['Boot_Timeout_Errors', 'Resource_Allocation_Errors', 'Network_Connection_Errors', 
            ...
            for category in priority_errors:
                category_logs = error_subcats[error_subcats['final_category'] == category]
                if len(category_logs) > 0:
                    # Boost these categories significantly
                    min_samples = 25 if category in ['Boot_Timeout_Errors', 'Resource_Allocation_Errors'] else 20
                    sample_size = min(min_samples, len(category_logs))
                    llm_sample.append(category_logs.sample(n=sample_size, random_state=42))
            ...
            other_errors = error_subcats[~error_subcats['final_category'].isin(priority_errors)]
            
            if len(other_errors) > 0 and remaining_error_slots > 0:
                for category in other_errors['final_category'].unique():
                    category_logs = other_errors[other_errors['final_category'] == category]
                    ...
                    if sample_size > 0 and sum(len(s) for s in llm_sample) < target_errors:
                        llm_sample.append(category_logs.sample(n=sample_size, random_state=42))
        
        # NETWORK OPERATIONS (15% of LLM samples)
        network_ops_llm = llm_classified[llm_classified['final_category'] == 'Network_Operations']
        if len(network_ops_llm) > 0:
            target_network = int(llm_target * 0.15)  # 63 logs
            llm_sample.append(network_ops_llm.sample(n=min(target_network, len(network_ops_llm)), random_state=42))
        ...
        other_llm_cats = llm_classified[
            llm_classified['final_category'].isin([
                'Resource_Management', 'System_Operations', 'Instance_Management', 
                'Service_Communication_Errors', 'Configuration_Errors', 'Instance_Management_Compute'
            ])
        ]
        if len(other_llm_cats) > 0:
            ...
            if target_other > 0:
                ...
                boost_categories = ['Resource_Management', 'Instance_Management', 'Configuration_Errors']
                for boost_cat in boost_categories:
                    boost_logs = other_llm_cats[other_llm_cats['final_category'] == boost_cat]
                    if len(boost_logs) > 0 and target_other >= 20:
                        boost_size = min(20, len(boost_logs), target_other)
                        llm_sample.append(boost_logs.sample(n=boost_size, random_state=42))
                        target_other -= boost_size
                
                # Add some Instance_Management_Compute for category balance
                instance_mgmt_llm = other_llm_cats[other_llm_cats['final_category'] == 'Instance_Management_Compute']
                if len(instance_mgmt_llm) > 0 and target_other >= 15:
                    llm_sample.append(instance_mgmt_llm.sample(n=min(15, len(instance_mgmt_llm)), random_state=42))
                    target_other -= 15
                
                # Distribute across remaining categories
                remaining_cats = other_llm_cats[~other_llm_cats['final_category'].isin([
                    'Instance_Management_Compute', 'Resource_Management', 'Instance_Management', 'Configuration_Errors'
                ])]
                if len(remaining_cats) > 0 and target_other > 0:
                    ...
                    for category in other_categories.index:
                        category_logs = remaining_cats[remaining_cats['final_category'] == category]
        ...
        print(f"✅ LLM showcase: {len(llm_final)} logs with {len(llm_final['final_category'].unique())} categories")
    ...
    if len(unclassified) > 0:
        ...
        unclassified_sample = unclassified.sample(n=min(unclassified_target, len(unclassified)), random_state=42)
    ...
    if len(final_demo_dataset) > target_size:
        ...
        if regex_current < regex_target:
            ...
            regex_available = available_logs[available_logs['pipeline_stage'] == 'Regex']
            if len(regex_available) > 0:
                additional_regex = regex_available.sample(n=min(regex_needed, len(regex_available)), random_state=42)
        ...
        if bert_current < bert_target and remaining_needed > 0:
            ...
            if len(bert_available) > 0:
                additional_bert = bert_available.sample(n=min(bert_needed, len(bert_available)), random_state=42)
        ...
        if llm_current < llm_target and remaining_needed > 0:
            ...
            if len(llm_available) > 0:
                additional_llm = llm_available.sample(n=min(llm_needed, len(llm_available)), random_state=42)
        ...
        if remaining_needed > 0:
            remaining_available = available_logs[~available_logs.index.isin(final_demo_dataset.index)]
            if len(remaining_available) > 0:
                additional_sample = remaining_available.sample(n=min(remaining_needed, len(remaining_available)), random_state=42)
...
def analyze_demo_dataset(demo_dataset):
    ...
    # ERROR SUBCATEGORIZATION SHOWCASE - Your Key Innovation
    error_cats = demo_dataset[demo_dataset['final_category'].str.contains('Error|Timeout', na=False)]
    if len(error_cats) > 0:
        print(f"\n🚀 ERROR SUBCATEGORIZATION INNOVATION:")
        print(f"   Total error logs: {len(error_cats)} ({len(error_cats)/len(demo_dataset)*100:.1f}%)")
    ...
    if len(conf_logs) > 0:
        ...
        print(f"  High confidence (≥0.8): {len(conf_logs[conf_logs['final_confidence'] >= 0.8]):3} logs")
        print(f"  Medium confidence (0.7-0.8): {len(conf_logs[(conf_logs['final_confidence'] >= 0.7) & (conf_logs['final_confidence'] < 0.8)]):3} logs")
        print(f"  Low confidence (<0.7): {len(conf_logs[conf_logs['final_confidence'] < 0.7]):3} logs")
    
    # Original label distribution (to show data quality)
...
def main():
    ...
    # Load the complete hybrid pipeline results
    ...
    # Create the production demo dataset
    demo_dataset = create_production_demo_from_complete_results(df_complete, target_size=2000)
